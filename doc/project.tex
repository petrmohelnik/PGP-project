...
\begin{itemize}
  \item Petr Mohelník\,--\,simulace dýmu
  \item Tomá¹ Rù¾ièka\,--\,vykreslení dýmu
\end{itemize}
...
Pro implementaci jsme pou¾ili \textit{C++}, \textit{STL}, \textit{SDL}, \textit{GLM}, \textit{GLEW} a \textit{OpenGL} ve verzi {\ttfamily 4.3}. K simulaci dýmu byly vyu¾ity \textit{compute shadery} spolu s \textit{shader storage} buffery. V èásti stínovány zaji¹»ovaly \textit{framebuffery} záznam do textur.

\subsection{Simulace}
Pro simulaci je zavedeno nìkolik  shader storage buffer objektù (SSBO), v¹echny mají velikost maximálního poètu èástic v systému. Pro vytvoøení základní struktury èásticového systému jsme se inspirovali ve slajdech z pøedná¹ky \cite{thomas}.
...
Implementovali jsme nìkolik metod pro správnou simulaci a zobrazení dýmu. Pro simulaci byly pou¾ity metody \textit{Smoothed Particle Hydrodynamics} a \textit{Bitonic sort}. Pøi vykreslení pak byly pou¾ity techniky \textit{shadow mappingu}, \textit{light bufferu} a \textit{sun rays}. Výsledek byl testován na sestavì s grafikou: \textit{ASUS Strix GeForce GTX 960 2GB}. Bohu¾el se nepodaøilo zmìøit pøesný poèet snímkù za sekundu, proto¾e pøi testování poèet neklesnul pod 60 snímkù za sekundu. Zbytek namìøeých hodnot shrnuje následující tabulka:
...
\begin{center}
  \begin{tabular}{|c|c|c|c|}
    \hline
    Doba bìhu & Vyu¾ití GPU [\%] & Poèet èástic & FPS \\
    \hline
    10 s & 30 & 2500 & 60 \\
    30 s & 35 & 8000 & 60 \\
    1 m & 40 & 16000 & 60 \\
